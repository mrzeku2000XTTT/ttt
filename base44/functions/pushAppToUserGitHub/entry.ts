...
export default async function(req) {
  try {
    ...
    if (!token || !repo || !Array.isArray(files) || !files.length) {
      return Response.json({ error: "Missing token, repo, or files" }, { status: 400 });
    }

    // 1. Validate the token + learn the GitHub user's login
    const meRes = await fetch(`${GH}/user`, { headers: ghHeaders(token) });
    if (!meRes.ok) {
      const t = await meRes.text();
      return Response.json({ error: `GitHub rejected your token (${meRes.status}). ${t.slice(0, 180)}` }, { status: 401 });
    }
    ...
    if (!repoName) return Response.json({ error: "Invalid repo name" }, { status: 400 });

    // 3. Ensure the repo exists (create it under the token's user if missing)
    ...
    if (repoRes.status === 404) {
      ...
      if (!createRes.ok) {
        ...
        return Response.json({ error: `Could not create repo (${createRes.status}). ${t.slice(0, 180)}` }, { status: 502 });
      }
      repoRes = await fetch(`${GH}/repos/${owner}/${repoName}`, { headers: ghHeaders(token) });
    }
    if (!repoRes.ok) {
      const t = await repoRes.text();
      return Response.json({ error: `Could not access repo "${owner}/${repoName}" (${repoRes.status}). ${t.slice(0, 180)}` }, { status: 502 });
    }
    const repoData = await repoRes.json();
    const defaultBranch = repoData.default_branch || "main";
    const targetBranch = (branch || "").trim() || defaultBranch;

    // 4. Resolve the target branch ref (create it off the default branch if missing)
    let parentSha = null;
    ...
    if (refRes.ok) {
      ...
      if (!defRefRes.ok) {
        return Response.json({ error: `Default branch "${defaultBranch}" not found.` }, { status: 502 });
      }
      ...
      if (!createBranchRes.ok) {
        ...
        return Response.json({ error: `Could not create branch "${targetBranch}". ${t.slice(0, 180)}` }, { status: 502 });
      }
    } else {
      const t = await refRes.text();
      return Response.json({ error: `Could not read branch "${targetBranch}". ${t.slice(0, 180)}` }, { status: 502 });
    }
    ...
    for (const f of files) {
      ...
      if (!blobRes.ok) {
        ...
        return Response.json({ error: `Failed to upload "${path}". ${t.slice(0, 180)}` }, { status: 502 });
      }
      treeEntries.push({ path, mode: "100644", type: "blob", sha: (await blobRes.json()).sha });
    }
    if (!treeEntries.length) {
      return Response.json({ error: "No valid files to push." }, { status: 400 });
    }
    ...
    if (!treeRes.ok) {
      ...
      return Response.json({ error: `Failed to create git tree. ${t.slice(0, 180)}` }, { status: 502 });
    }
    ...
    if (!commitRes.ok) {
      ...
      return Response.json({ error: `Failed to create commit. ${t.slice(0, 180)}` }, { status: 502 });
    }
    const newCommit = await commitRes.json();

    // 8. Move the branch ref forward to the new commit
    ...
    if (!updateRes.ok) {
      ...
      return Response.json({ error: `Commit created but branch update failed. ${t.slice(0, 180)}` }, { status: 502 });
    }
