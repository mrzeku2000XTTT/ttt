...
Deno.serve(async (req) => {
  try {
    ...
    if (action === "send") {
      ...
      const sender = byName[from === "beta" ? "beta" : "alpha"];
      ...
      if (balance < amt) {
        return Response.json({
          error: `Agent ${from === "beta" ? "beta" : "alpha"} has insufficient iKAS. Balance: ${ethers.formatEther(balance)} iKAS — fund ${sender.address} first.`,
        }, { status: 400 });
      }
      ...
      return Response.json({
        ...
        from_agent: from === "beta" ? "beta" : "alpha",
        from: sender.address,
        to: dest,
