...
export default function CameraStudio() {
  ...
  const [playhead, setPlayhead] = useState(0), [armed, setArmed] = useState(false), [activePointId, setActivePointId] = useState(null), [pointPreview, setPointPreview] = useState(null);
  ...
  return <div className="camera-studio">
    ...
    <main className="camera-body">
      {!state.loaded ? <div role="status" className="flex items-center gap-2"><Loader2 size={18} className="animate-spin"/>Opening local project…</div> : <CameraStudioStage asset={asset} settings={project.settings} playing={playing} engineRef={engine} add={add} upload={upload} busy={exporting} onReady={setReady} onError={setError} interestPoints={previewPoints} playhead={playhead} onTime={setPlayhead} armed={project.editorMode === 'composer' && armed} activePointId={activePointId} onSelectPoint={selectInterestPoint} onPlacePoint={placeInterestPoint} onPreviewPoint={previewInterestPoint} onMovePoint={updateInterestPoint} showFocus={project.settings.mode === 'video' && project.editorMode === 'composer'}/>}
