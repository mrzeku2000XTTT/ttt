...
export default function CameraStudio() {
  ...
  const [playhead, setPlayhead] = useState(0), [armed, setArmed] = useState(false), [activePointId, setActivePointId] = useState(null), [pointPreview, setPointPreview] = useState(null), [layerPreview, setLayerPreview] = useState(null), [recording, setRecording] = useState(false), [drawMode, setDrawMode] = useState(false);
  ...
  const resetWorkspace = () => { setPlaying(false); setRecording(false); setDrawMode(false); setPlayhead(0); setLibrary(false); state.newProject(); };
  const capture = async () => {
  ...
  return <div className="camera-studio">
    ...
    <CameraStudioToolbar project={project} rename={name => state.patch({ name })} add={upload} library={library} toggleLibrary={() => setLibrary(!library)} undo={() => { setPlaying(false); state.undo(); }} redo={() => { setPlaying(false); state.redo(); }} canUndo={state.past.length > 0} canRedo={state.future.length > 0} newProject={resetWorkspace} deleteProject={() => { setPlaying(false); state.deleteProject(project.id); }} playing={playing} togglePlay={() => setPlaying(!playing)} disabled={exporting || !state.loaded} canPlay={ready} status={state.status} editorMode={project.editorMode || 'operator'} setEditorMode={editorMode => { setPlaying(false); setArmed(false); setPlayhead(0); state.patch({ editorMode }); }} compose={() => { setArmed(false); setPlayhead(0); state.patch({ editorMode: 'compose' }); setPlaying(true); }} videoMode={project.settings.mode === 'video'} armed={armed} toggleArmed={() => { setPlaying(false); state.patch({ editorMode: 'operator' }); setArmed(value => !value); }} pointCount={interestPoints.length} drawMode={drawMode} toggleDraw={() => { setPlaying(false); setArmed(false); setDrawMode(value => !value); }} canDelete={interestPoints.some(point => point.id === activePointId)} deletePoint={() => removeInterestPoint(activePointId)}/>
