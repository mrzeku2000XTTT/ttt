...
export default function FrivPage() {
  ...
  const [expiresAt, setExpiresAt] = useState(() => {
    const saved = localStorage.getItem('friv_expires_at');
  ...
  const checkSubscription = () => {
    ...
    if (remaining <= 0) {
      ...
      setShowPaymentModal(true);
      setTimeRemaining(0);
      localStorage.removeItem('friv_expires_at');
      setExpiresAt(null);
  ...
  const addTime = () => {
    ...
    localStorage.setItem('friv_expires_at', newExpiry.toString());
    setGameUnlocked(true);
  };
  ...
  return (
    <>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white font-bold text-lg">{expiresAt && expiresAt > Date.now() ? 'Add More Time' : 'Unlock FRIV'}</h3>
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-white/80">
                      <p className="mb-2">Pay <span className="font-bold text-yellow-400">1 KAS</span> to yourself to {expiresAt && expiresAt > Date.now() ? 'add' : 'unlock FRIV for'} <span className="font-bold text-cyan-400">20 minutes</span>.</p>
          ...
          >
            ...
            >
              ...
              <p className="text-white/60 text-sm mb-6">
                Send KAS to yourself in Kaspium to unlock FRIV
              </p>
      ...
      <div className="fixed left-0 right-0 bottom-0 bg-black flex flex-col" style={{ top: 'calc(var(--sat, 0px) + 7.5rem)' }}>
        {/* Header */}
        <div className="flex-none bg-black/80 backdrop-blur-xl border-b border-white/10 p-4 flex items-center justify-between gap-4">
        ...
        {gameUnlocked ? (
          ...
          <div className="flex-1 flex items-center justify-center bg-black">
            <div className="text-center">
              ...
              <p className="text-white/80 text-lg mb-2">FRIV is locked</p>
              <p className="text-white/40 text-sm">Pay 1 KAS for 20 minutes of playtime</p>
            </div>
          </div>
        )}
      </div>
