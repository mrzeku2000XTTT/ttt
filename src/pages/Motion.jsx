...
import { ArrowLeft, Loader2, Lock, X, Users } from "lucide-react";
...
import MotionCommunityCreations from "@/components/motion/MotionCommunityCreations";
...
export default function MotionPage() {
  ...
  const [mobileTab, setMobileTab] = useState("prompt"); // 'prompt' | 'code' | 'community'
...
${prompt}`,
  ...
  const handleSaveCommunity = async () => {
    ...
    const title = (activePreset?.name && activePreset.name !== "seeded")
      ? activePreset.name
      : prompt.split("\n").find(Boolean)?.slice(0, 60) || "Motion Creation";

    await base44.entities.MotionCreation.create({
      title,
      prompt,
      code,
      creator_email: user?.email || "",
      creator_name: user?.full_name || user?.username || "",
      preset_name: activePreset?.name || "Custom",
      is_public: true,
    });
  };

  const handleLoadCommunityCreation = (creation) => {
    setPrompt(creation.prompt || "");
    setCode(creation.code || "");
    setActivePreset({
      id: `community-${creation.id}`,
      name: creation.title || "Community Creation",
      tagline: "Loaded from community",
    ...
    <div className="fixed inset-0 bg-[#0a0a0f] text-white flex flex-col">
      ...
      <nav className="h-14 flex items-center justify-between px-5 bg-black/60 backdrop-blur-xl border-b border-white/10 z-40 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`hidden sm:flex items-center gap-1.5 h-8 px-3 rounded-lg text-[11px] font-bold border transition-all ${
              mobileTab === "community"
        ...
        <span className="text-[11px] text-white/40 font-medium hidden sm:block">GitHub sync runs from Base44 Dashboard</span>
      </nav>
      ...
      {mobileTab === "community" ? (
        <div className="flex-1 min-h-0">
          <MotionCommunityCreations onLoadCreation={handleLoadCommunityCreation} />
        </div>
      ) : (
      <>
