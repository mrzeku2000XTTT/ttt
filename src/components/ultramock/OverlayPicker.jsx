 ...
 * Desktop: centered modal that fades/scales in from the middle of the screen.
 */
export default function OverlayPicker({ open, onClose, onPickPreset, onPickImage }) {
  ...
  // The shared body — same content used by both the mobile sheet and desktop modal
  const body = (
    <>
      ...
      <div className="flex items-center gap-1 px-3 pt-3">
        <button
          ...
          className={`flex items-center gap-1.5 h-9 px-3 rounded-lg text-xs font-bold transition-colors ${
            tab === "library" ? "bg-white text-black" : "bg-white/5 text-white/60 hover:bg-white/10"
          }`}
        >
          ...
          <div className="space-y-3">
            ...
            {referenceUrl ? (
              <div className="flex items-center gap-2 p-2 rounded-lg bg-white/5 border border-fuchsia-400/30">
                <img src={referenceUrl} alt="reference" className="w-12 h-12 rounded object-cover flex-shrink-0" />
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="sm:hidden fixed inset-x-0 bottom-0 z-[71] bg-zinc-950 border border-white/10 rounded-t-3xl shadow-2xl flex flex-col"
            style={{
              maxHeight: "85vh",
          ...
          {/* Desktop: centered modal that pops in from the middle */}
          <div
            className="hidden sm:flex fixed inset-0 z-[71] items-center justify-center pointer-events-none"
          >
            <motion.div
              ...
              className="pointer-events-auto w-[640px] max-w-[92vw] bg-zinc-950 border border-white/10 rounded-3xl shadow-2xl flex flex-col"
