...
export default function KilnComponentRail({ used = [], onInstruct }) {
  const usedNames = new Set(used.map((entry) => entry.component));
  return (
    <aside className="kiln-card kiln-scroll flex min-h-0 flex-col overflow-y-auto">
      ...
      <div className="space-y-[3px] p-2">
        {ETA_LIBRARY.map((component) => {
          ...
          return (
            <button
              key={component.name}
              onClick={() => onInstruct(`Rebuild the main block as an ETA ${component.name} component, keeping the current content and styling.`)}
