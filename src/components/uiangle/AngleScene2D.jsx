...
export default function AngleScene2D({ editor:e, preview=false }) {
  ...
  return <div className={`relative bg-background angle-grid ${preview?'aspect-video':'flex h-full min-h-[390px] items-center p-5 lg:min-h-0'}`}>
    <svg ref={svg} viewBox="0 0 1600 900" className="aspect-video w-full touch-none overflow-hidden bg-card" onPointerMove={move} onPointerUp={end} onPointerCancel={end} onLostPointerCapture={()=>{drag.current=null;}}>
      <g transform={cameraTransform}>
        {e.reference&&<image href={e.reference.url} width="1600" height="900" opacity=".25" preserveAspectRatio="xMidYMid meet"/>}
      ...
      {!preview&&<g transform={`translate(${c.x*16} ${c.y*9}) rotate(${c.roll})`}><rect x={-w/2} y={-h/2} width={w} height={h} fill="none" className="stroke-foreground" opacity=".65" strokeWidth="2" strokeDasharray="10 10" pointerEvents="none"/><g transform="translate(0 0)" onPointerDown={event=>start(event,'camera')} className="cursor-move"><rect x="-45" y="-17" width="90" height="34" rx="7" className="fill-foreground"/><text textAnchor="middle" y="6" fontSize="16" className="fill-background">CAMERA</text></g></g>}
    </svg>
