...
export default function useAngleActions(editor) {
  const [busy,setBusy]=useState(''),[error,setError]=useState(''),[result,setResult]=useState(null),[showResult,setShowResult]=useState(false),[copied,setCopied]=useState(false),lock=useRef(false);
  ...
  const generate=async()=>{
    ...
    try {
      ...
      if(!response.url)throw new Error('No image was returned. Please try again.');setResult({url:response.url,prompt});setShowResult(true);
  ...
  return {busy,error,result,copied,showResult,upload,generate,copy,closeResult:()=>setShowResult(false),openResult:()=>setShowResult(true)};
}