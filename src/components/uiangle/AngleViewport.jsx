...
export default function AngleViewport({ editor, preview=false, captureRef }) {
  ...
  return <div className={`relative overflow-hidden bg-background ${preview?'aspect-video':'h-full min-h-[390px]'}`}><div ref={host} className="absolute inset-0"/>{error&&<p role="alert" className="absolute inset-x-4 top-12 rounded bg-card p-3 text-xs">{error}</p>}{!preview&&<><div className="pointer-events-none absolute left-4 top-4 text-[10px] uppercase tracking-[.18em] text-muted-foreground">Perspective / scene view</div><button title="Reset workspace view" onClick={()=>engine.current?.reset()} className="absolute right-4 top-4 rounded-lg border border-border bg-card p-2"><RotateCcw size={14}/></button><p className="pointer-events-none absolute bottom-4 left-4 text-[11px] text-muted-foreground">Drag subject or camera · Drag empty space to orbit · Scroll to zoom</p></>}</div>;
}