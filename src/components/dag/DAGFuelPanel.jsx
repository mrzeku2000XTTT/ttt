...
export default function DAGFuelPanel({ stats, onDonate }) {
  ...
  const loadDonations = async () => {
    try {
      const res = await base44.functions.invoke('getDAGDonations', {});
      if (res.data?.totalDonated) {
        setTotalDonated(res.data.totalDonated);
      }
    } catch (err) {
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative cursor-default">
          <div className="flex flex-col gap-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg min-w-[120px]">
            <span className="font-mono text-[9px] uppercase text-white/50">Donated</span>
            <div className="font-mono text-sm font-bold text-white">
              {totalDonated.toFixed(2)} KAS
            </div>
          </div>
        </div>
