...
export default function GameTimer({ endTime }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-white/30 text-[10px] font-mono flex-shrink-0">{ctStr} CT</span>
      <div className="relative flex-1 h-1 bg-white/[0.06] rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-1000 ${isUrgent ? 'bg-rose-500' : 'bg-violet-500'}`}
          style={{ width: `${pct}%` }}
        />
      </div>
      <div className={`flex items-center gap-1.5 flex-shrink-0 ${isUrgent ? 'text-rose-400' : 'text-white/70'}`}>
