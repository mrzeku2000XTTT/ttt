...
import { Search, Plus, Plug, ExternalLink, Check } from "lucide-react";
...
export default function DDStore() {
  ...
  const filtered = DD_STORE_APPS.filter(
          ...
          return (
            <div key={a.name} className="bg-white border border-neutral-200 rounded-2xl p-4 flex flex-col">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  onClick={() => setAdded((s) => ({ ...s, [a.name]: true }))}
                  className={`h-8 px-3 rounded-lg text-xs font-medium flex items-center gap-1.5 transition ${isAdded ? "bg-emerald-50 text-emerald-600 border border-emerald-200" : st === "open" ? "bg-white text-neutral-900 border border-neutral-200 hover:border-neutral-300" : "bg-neutral-900 text-white"}`}
                >
                  {isAdded ? <><Check className="w-3.5 h-3.5" /> Added</> : st === "open" ? <><ExternalLink className="w-3.5 h-3.5" /> Open</> : st === "connect" ? <><Plug className="w-3.5 h-3.5" /> Connect</> : <><Plus className="w-3.5 h-3.5" /> Add</>}
                </button>
              </div>
            </div>
          );
