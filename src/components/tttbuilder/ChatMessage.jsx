...
function Collapsible({ icon: Icon, label, children, defaultOpen = false }) {
  ...
  return (
    <div className="mt-2 border-t border-black/[0.06] pt-2">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 text-[11px] font-bold text-[#86868B] hover:text-[#1D1D1F] transition-colors"
      >
...
export default function ChatMessage({ message }) {
  ...
  if (isUser) {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl px-4 py-2.5 text-sm whitespace-pre-wrap bg-[#007AFF] text-white">
          {message.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-start">
      <div className="max-w-[92%] w-full rounded-2xl px-4 py-3 text-sm bg-white border border-black/[0.06] shadow-[0_1px_3px_rgba(0,0,0,0.04)] text-[#1D1D1F]">
        ...
        {thinking.length > 0 && (
          <Collapsible icon={Brain} label={`Thoughts (${thinking.length})`}>
            <ol className="space-y-1.5">
              {thinking.map((t, i) => (
                <li key={i} className="flex gap-2 text-[11px] text-[#86868B] leading-relaxed">
        ...
        {files.length > 0 && (
          <Collapsible icon={FolderOpen} label={`${files.length} file${files.length > 1 ? "s" : ""} written`}>
            <ul className="space-y-1">
              {files.map(f => (
                <li key={f} className="text-[11px] font-mono text-[#86868B] truncate">{f}</li>
              ))}
            </ul>
          </Collapsible>
        )}
