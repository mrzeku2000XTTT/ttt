...
export default function BuilderLandingBanner() {
  return (
    <section className="relative overflow-hidden bg-black border-y border-[#00ff99]/15">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <p className="text-[11px] font-black tracking-[0.2em] text-[#00ff99] mb-4">THE KASPA ECONOMY</p>
          <h2 className="text-3xl sm:text-4xl xl:text-5xl font-bold tracking-[-0.02em] leading-[1.05] text-white mb-5">
            More than a wallet. A full <span className="text-[#00ff99]">economic</span> layer.
          </h2>
          ...
          <a
            href="/AppStoreV2"
            className="inline-flex items-center gap-2 h-11 px-6 rounded-full bg-white text-black text-sm font-bold hover:bg-[#00ff99] transition-colors"
          >
            Explore the Ecosystem <ArrowRight className="w-4 h-4" />
          </a>
        </div>

        {/* Official Kaspa mark with floating layer cards */}
        <div className="relative h-72 sm:h-96 hidden sm:block">
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="absolute w-56 h-56 rounded-full border border-[#00ff99]/20" />
            <div className="absolute w-72 h-72 rounded-full border border-[#00ff99]/10" />
            <KaspaMark size={64} glow />
          </div>
          {ORBS.map((o, i) => (
            <motion.div
              key={o.label}
              ...
              className={`absolute ${o.className} px-4 py-2.5 rounded-xl bg-black/60 backdrop-blur-md border border-[#00ff99]/30 text-white text-xs font-bold`}
            >
              {o.label}
            </motion.div>
          ))}
