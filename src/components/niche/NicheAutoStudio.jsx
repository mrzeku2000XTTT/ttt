...
${linkContext ? `\nThe user pasted a link. Its actual fetched content:\n"""${linkContext}"""\nHARD RULE for pasted links: the video's topic is exactly the subject of this fetched content — never the URL, the account name, one of the user's saved niches, or anything found in web search that is not this content. If it is an X/Twitter post: build a structured, scene-by-scene explainer OF THE POST ITSELF — its subject (the project, character, update or event the post announces), its points in the order the post makes them, and every claim in the script traceable to the post text. 1:1 accuracy with the post — and cover ALL of the post's points across the scenes so the video feels complete end to end, never a teaser. Do NOT pivot to a generic explainer, an ecosystem overview, a competitor or loosely related project, or a random niche — derive the video's angle and niche from the post's own content. STYLE for X posts — MANDATORY REMIX: unless the user explicitly asked for one specific style, you MUST set the "style" field on EVERY scene (a missing style field falls back to one uniform look, which is wrong for X posts). Use at least 3 different style ids across the video and never let more than half the scenes share the same style — you have the full animation canon to draw from (cel, digital-2d, cgi-3d, anime, claymation, cutout, motion-graphics, rotoscope, rubber-hose, hybrid-2d, vox, real-ui, comic, papercut, isometric, chalkboard, neutral). Pick per scene from that moment's own content: cinematic/narrative beat → vox or anime, app/wallet UI moment → real-ui (+ app field), emotional beat → hybrid-2d or anime, charming moment → claymation or cutout, retro/playful beat → rubber-hose or cel, sleek tech beat → motion-graphics or cgi-3d, realistic human moment → rotoscope, plain explainer beat → neutral, diagram beat → isometric or chalkboard.` : (pastedUrls.some((u) => /(?:x|twitter)\.com\/./i.test(u)) ? `\nThe user pasted an X/Twitter link but its content could not be fetched. HARD RULE: research THAT EXACT post live — search its author, the post's subject and its exact claims — and build the video about exactly what that post announces, never a generic explainer, an ecosystem overview or a loosely related project. Cover every point the post makes, and apply the mandatory per-scene style remix (at least 3 different style ids across the video).` : '')}
