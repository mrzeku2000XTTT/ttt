...
export default function SearchProfileTab({ onOpenDocs }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-3 px-3 pb-4 pt-4">
      <header className="flex items-baseline gap-2">
        <h1 className="text-xl font-bold text-white">Profile</h1>
        <p className="text-[11px] text-white/40">Your wallet, your profile, your listings.</p>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <section className="space-y-2 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-semibold text-white">Write your own profile</h2>
          </div>
        ...
        <section className="space-y-2 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-semibold text-white">List a site</h2>
          </div>
          <p className="text-[11px] leading-snug text-white/40">
            Own a Kaspa project or site? It gets security-scanned and added to the search index.
          </p>
          <button onClick={() => setListOpen(true)} className="h-9 w-full rounded-xl bg-cyan-500 text-[13px] font-bold text-black transition-transform active:scale-95">
            List your site
          </button>
        </section>
      </div>
      ...
      <button
        onClick={onOpenDocs}
        className="flex w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-left transition-colors hover:bg-white/[0.07] active:scale-[0.99]"
      >
