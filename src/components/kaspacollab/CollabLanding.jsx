...
export default function CollabLanding({ onConnect, loading, error, hasWallet }) {
  return (
    <div className="kaspa-collab-landing kc-page relative min-h-screen overflow-hidden bg-background text-foreground">
      <div className="relative mx-auto max-w-[1240px] px-5 sm:px-10">
        <header className="relative z-30 flex min-h-[88px] items-center justify-between gap-5">
          ...
          <div className="flex shrink-0 items-center gap-4">
            <a href="/AppStoreV2" className="text-[11px] text-muted-foreground transition hover:text-foreground">Exit to Store</a>
            <button onClick={onConnect} disabled={loading} className="kc-button hidden sm:inline-flex">{loading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wallet className="h-3.5 w-3.5" />}{loading ? 'Connecting…' : hasWallet ? 'Enter workspace' : 'Connect wallet'}</button>
          </div>
        </header>
        <main>
          <CollabLandingHero onConnect={onConnect} loading={loading} error={error} hasWallet={hasWallet} />
          <CollabLandingWorkspace onConnect={onConnect} loading={loading} hasWallet={hasWallet} />
          <CollabLandingIdeas onConnect={onConnect} loading={loading} hasWallet={hasWallet} />
        </main>
      </div>
    </div>
  );
