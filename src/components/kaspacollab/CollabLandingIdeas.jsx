...
export default function CollabLandingIdeas({ onConnect, loading }) {
  return (
    <section id="collab-ideas" className="scroll-mt-8 pb-16 pt-20 sm:pt-24">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-6">{ideas.map(([title, text, Icon], index) => <article key={title} className="kc-idea-card flex flex-col rounded-2xl border border-border p-4">
        ...
        <button onClick={onConnect} disabled={loading} className="kc-button mt-auto w-full !px-2 !py-2 !text-[10px]">{loading ? 'Connecting…' : 'Start a session'}<ArrowUpRight className="h-3 w-3" /></button>
      </article>)}</div>
    </section>
  );
}