...
export default function ClutchPlayer({clip,active}){
  ...
  if(clip.source_type==='youtube')return active?<div className="clutch-youtube"><iframe title={clip.title} src={`https://www.youtube.com/embed/${clip.youtube_id}?autoplay=1&mute=1&playsinline=1&rel=0&loop=1&playlist=${clip.youtube_id}`} allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/><a target="_blank" rel="noreferrer" href={`https://www.youtube.com/watch?v=${clip.youtube_id}`}>Can’t play? Open on YouTube</a></div>:<img className="clutch-poster" src={`https://i.ytimg.com/vi/${clip.youtube_id}/hqdefault.jpg`} alt={clip.title}/>;
  ...
  if(!source.data)return <div className="clutch-player-message"><Loader2 className="animate-spin"/><span>Loading highlight…</span></div>;
