...
export default function ETAScenePlan({ plan, onBack, onSceneChange, onAddScene, onDeleteScene, onCompile, onEdit }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-10">
      ...
      <div className="space-y-4">{plan.scenes.map((scene, index) => <ETASceneCard key={index} scene={scene} index={index} onChange={(next) => onSceneChange(index, next)} onDelete={() => onDeleteScene(index)} />)}</div>
