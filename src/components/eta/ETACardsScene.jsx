...
export default function ETACardsScene({ scene, advanced }) {
      ...
      {Array.from({ length: count }).map((_, i) => {
        ...
        return (
          <motion.div
            ...
            animate={{ x: [x, x + (i % 2 ? 8 : -8), x], y: [22, 8, 22], rotate: [rotate, rotate + (i % 2 ? 4 : -4), rotate], opacity: 1 }}
            transition={{ opacity: { delay: i * .12 }, x: { duration, repeat: Infinity, ease: 'easeInOut' }, y: { duration, repeat: Infinity, ease: 'easeInOut' }, rotate: { duration, repeat: Infinity, ease: 'easeInOut' } }}
            ...
            style={{ borderRadius: radius, backgroundColor: advanced.cardColor }}
          >
