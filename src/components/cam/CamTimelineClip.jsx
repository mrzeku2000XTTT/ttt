...
export default function CamTimelineClip({ clip, image, title, zoom, selected, onSelect, onUpdate, disabled, camera, onSeek }) {
  ...
  return <div role="button" tabIndex={0} className={`cm-tl-clip ${camera?'is-scene':''} ${selected?'is-selected':''}`} style={{left:clip.start*zoom,width:Math.max(12,clip.duration*zoom)}} title={`${title} · ${clip.start.toFixed(2)}–${(clip.start+clip.duration).toFixed(2)}s · ${clip.keys.length} keyframes`} onClick={(e)=>{e.stopPropagation();onSelect(clip);}} onPointerDown={(e)=>begin(e,'move')} onPointerMove={move} onPointerUp={()=>{drag.current=null;}} onPointerCancel={()=>{drag.current=null;}}>
    ...
    <div className="cm-tl-keys">{clip.keys.filter((k)=>k.t<=clip.duration).map((key,i)=><i key={i} style={{left:Math.min(99,key.t/clip.duration*100)+'%'}} role={camera?'button':undefined} tabIndex={camera&&!disabled?0:undefined} title={camera?`Camera keyframe · ${(clip.start+key.t).toFixed(2)}s`:undefined} onPointerDown={camera?e=>e.stopPropagation():undefined} onClick={camera?e=>{e.stopPropagation();if(!disabled)onSeek?.(clip.start+key.t);}:undefined} onKeyDown={camera?e=>{if(!disabled&&(e.key==='Enter'||e.key===' ')){e.preventDefault();e.stopPropagation();onSeek?.(clip.start+key.t);}}:undefined}/>)}</div>
  </div>;
}