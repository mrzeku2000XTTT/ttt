...
import { moveById } from '@/components/cam/camMoves';
...
export default function CamTimelineRow({ track, timeline: t, media, width, zoom, camera = false, onSelectAsset }) {
  ...
  return <><div className={`cm-tl-row ${track.hidden ? 'is-muted' : ''}`}>
    ...
    <div className="cm-tl-lane" style={{ width }}>
      {track.clips.map((clip, i) => <CamTimelineClip key={clip.id} camera={camera} clip={clip} image={asset?.url || (camera ? media[0]?.url : undefined)} title={camera ? `${i + 1} · ${moveById(clip.keys?.[0]?.moveId).label}` : track.name} zoom={zoom} selected={t.selected === clip.id} disabled={t.recording} onUpdate={t.update} onSeek={time=>{t.setSelected(clip.id);t.seek(time);}} onSelect={(c) => { t.setSelected(c.id); t.seek(c.start); if (!camera) onSelectAsset(track.assetId); }} />)}
    </div>
