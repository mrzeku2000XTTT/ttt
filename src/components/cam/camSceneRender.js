...
export default function renderCamScene(canvas, scene, media, framing = 1) {
  ...
  const assets = scene.assets.map((a) => ({ ...a, img: media.find((m) => m.id === a.id)?.img })).filter((a) => a.img);
  ...
  for (const a of assets) {
    const project = (u, v) => { const p = new THREE.Vector3(a.x + (u - 0.5) * 3.4 * a.scale, a.y + (0.5 - v) * 1.9 * a.scale, a.z).project(camera); return { x: W / 2 + p.x * W / 2 * framing, y: H / 2 - p.y * H / 2 * framing, z: p.z, u: u * a.img.naturalWidth, v: v * a.img.naturalHeight }; };
