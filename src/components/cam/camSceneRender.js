...
export default function renderCamScene(canvas, scene, media, framing = 1) {
  ...
  for (const a of assets) {
    ...
    // Each patch only draws its own sub-rectangle of the source image —
    // drawing the full image 128× per frame made dragging extremely slow.
    const pw = a.img.naturalWidth / 8, ph = a.img.naturalHeight / 8;
    for (let y = 0; y < 8; y++) for (let x = 0; x < 8; x++) {
      ...
      for (const indices of [[0, 1, 2], [0, 2, 3]]) {
        ...
        if (!Number.isFinite(det) || !det) continue;
        ...
        ctx.setTransform(aa, bb, cc, dd, A.x - aa * A.u - cc * A.v, A.y - bb * A.u - dd * A.v); ctx.drawImage(a.img, x * pw, y * ph, pw, ph, x * pw, y * ph, pw, ph); ctx.restore();
      }
    }
