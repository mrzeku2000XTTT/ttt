...
export function captureScene(media, offset, rig, moveId, intensity, duration) {
  return { duration, camera: { ...rig, moveId, intensity }, assets: media.map((m) => ({ id: m.id, name: m.name, x: m.id === 'primary' ? offset.x * 2.4 : m.pos.x, y: m.id === 'primary' ? offset.y * 1.6 : m.pos.y, z: m.id === 'primary' ? offset.z * 2.2 : m.pos.z, scale: m.scale || 1 })) };
}
export function addScene(project, snapshot, start, duration, id = uid()) {
  ...
  for (const asset of snapshot.assets) {
    ...
    tracks[index] = { ...tracks[index], clips: [...tracks[index].clips, { id: uid(), takeId: id, start, duration, keys: [{ t: 0, x: asset.x, y: asset.y, z: asset.z, scale: asset.scale }] }] };
  }
...
export function recordSample(project, id, snapshot, elapsed) {
  ...
  return { cuts: project.cuts.map((c) => c.id === id ? { ...c, duration: Math.max(0.1, elapsed), keys: append(c.keys, snapshot.camera) } : c), tracks: project.tracks.map((track) => {
    ...
    return { ...track, clips: track.clips.map((c) => c.takeId === id && asset ? { ...c, duration: Math.max(0.1, elapsed), keys: append(c.keys, { x: asset.x, y: asset.y, z: asset.z, scale: asset.scale }) } : c) };
  }) };
}
