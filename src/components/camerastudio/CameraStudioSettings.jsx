...
export default function CameraStudioSettings({ settings, update, close }) {
  return <section className="camera-settings" aria-label="Camera settings">
    ...
    <label>Video duration<select value={settings.duration} onChange={e => update({ duration: Number(e.target.value) })}>{[4, 6, 10, 15].map(n => <option key={n} value={n}>{n} seconds</option>)}</select></label>
