...
export default function CameraStudioToolbar({ project, rename, add, library, toggleLibrary, undo, canUndo, playing, togglePlay, disabled, canPlay, status, editorMode, setEditorMode, compose, videoMode, armed, toggleArmed, pointCount, canDelete, deletePoint }) {
  return <div className="camera-toolbar">
    ...
    {videoMode ? <div className="camera-mode-switch">
      ...
      <button className="camera-interest-button" onClick={toggleArmed} aria-pressed={armed} disabled={disabled || !canPlay || pointCount >= 3} title="Set framing window"><Crosshair size={15}/>{pointCount > 0 && <span>{pointCount}</span>}</button><button onClick={deletePoint} disabled={!canDelete} title="Delete selected framing window"><Trash2 size={15}/></button>
