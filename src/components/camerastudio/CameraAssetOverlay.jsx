...
export default function CameraAssetOverlay({ assets, aspects, ratio, selected, select, preview, commit }) {
  ...
  const selectedAsset = assets.find(asset => asset.id === selected), motionPoints = (selectedAsset?.keyframes || []).map(frame => `${50 + (frame.transform?.x || 0) * 100},${50 + (frame.transform?.y || 0) * 100}`).join(' ');
  return <div className="camera-assets-overlay">{motionPoints && <svg className="camera-motion-path" viewBox="0 0 100 100" preserveAspectRatio="none"><polyline points={motionPoints}/></svg>}{assets.map(asset => { const box = frameBox(asset, aspects[asset.id] || 16 / 9, ratio); return <div key={asset.id} className={`camera-layer-box ${selected === asset.id ? 'is-selected' : ''}`} style={{ left: `${box.left}%`, top: `${box.top}%`, width: `${box.width}%`, height: `${box.height}%`, transform: `translate(-50%,-50%) rotate(${box.rotate}deg)` }} onPointerDown={event => begin(event, asset)} onPointerMove={move} onPointerUp={end}>{selected === asset.id && <button aria-label="Resize layer" className="camera-layer-resize" onPointerDown={event => begin(event, asset, true)} onPointerMove={move} onPointerUp={end}/>}</div>; })}</div>;
}