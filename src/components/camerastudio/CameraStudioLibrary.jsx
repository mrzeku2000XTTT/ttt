...
import { renderCameraTextCanvas } from '@/components/camerastudio/cameraTextTexture';
export default function CameraStudioLibrary({ assets, selected, select, remove, close, add, projects, currentProject, openProject, deleteProject }) {
  const items = useMemo(() => assets.map(asset => ({ ...asset, url: asset.type === 'text' ? renderCameraTextCanvas(asset).toDataURL('image/png') : URL.createObjectURL(asset.file) })), [assets]);
  useEffect(() => () => items.forEach(item => { if (item.type !== 'text') URL.revokeObjectURL(item.url); }), [items]);
  return <aside className="camera-library" aria-label="Media library">
    ...
    <div className="camera-library-grid">{items.map(item => <div className="camera-asset" key={item.id}>
      <button onClick={() => select(item.id)} aria-pressed={selected === item.id} title={item.name}>
        {item.type !== 'text' && item.file.type.startsWith('video/') ? <video src={item.url} muted playsInline preload="metadata"/> : <img src={item.url} alt={item.name}/>}
        <span>{item.name}</span>
